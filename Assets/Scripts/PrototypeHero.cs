...
public class PrototypeHero : MonoBehaviour {
    ...
    public int m_maxJump;
    public bool invincible;
    public float invincibleTime;
    public float invincibleIntTimer;
    public bool isParry;

    public Transform droneFollowPoint;
    public KeyFollow followingKey;
    public Transform attackPoint;
    public GameObject droneScale;
    ...
    void Update()
    {
        ...
        {
            //Up Attack
            if (Input.GetMouseButtonDown(0) && Input.GetKey("w") && !m_dodging && !m_ledgeGrab && !m_ledgeClimb && !m_crouching && m_grounded && m_timeSinceAttack > 0.2f)
            {
            ...
            //JumpInAir
            else if (Input.GetButtonDown("Jump") && (m_maxJump > 0) && !m_grounded && !m_dodging && !m_ledgeGrab && !m_ledgeClimb && !m_crouching && m_disableMovementTimer < 0.0f)
            {
                m_body2d.velocity = new Vector2(-m_facingDirection * m_jumpForce / 2.0f, m_jumpForce);
                m_facingDirection = -m_facingDirection;
                m_maxJump -= 1;
    ...
    public void Flip()
    {
        ...
        Vector3 drone = droneScale.transform.localScale;
        if (Input.GetAxis("Horizontal") > 0)
        {
            drone.x = -0.65f;
        ...
        droneScale.transform.localScale = drone;

    }

    private void OnTriggerEnter2D(Collider2D col)
    {
        if (col.gameObject.CompareTag("EnemySword"))
        {
            invincibleIntTimer = 1f;
        }
